        ...
        switch (parsedData.event) {
          ...
          case 'joinRoom': {
            ...
            if(room?.projectId){
              ...
              if(!project){
                ws.send(
                  JSON.stringify({ event: 'error', message: 'Project is not exist' }),
                );
                return;
              }
              const projectMember = await prisma.projectMember.findFirst({
                where: { projectId: room.projectId, memberId: ws.userId },
              });
              if (!projectMember) {
                ws.send(
                  JSON.stringify({ event: 'error', message: 'You are not allowed to join this room' }),
                );
                return;
              }
            }
