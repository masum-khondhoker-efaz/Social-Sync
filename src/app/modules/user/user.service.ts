...
const getAllUsersFromDB = async (userId: string, searchQuery?: string) => {
...
const updateUserRoleStatusIntoDB = async (userId: string, payload: any) => {
  ...
  if (!user) {
    throw new AppError(httpStatus.NOT_FOUND, 'User not found or available!');
  }
...
const forgotPassword = async (payload: { email: string }) => {
  ...
  await emailSender(
    ...
    <tr>
      <td style="background-color: #20B65E; padding: 20px; text-align: center; color: #f5f5f5; border-radius: 10px 10px 0 0;">
    ...
    <tr>
      <td style="padding: 20px;">
        ...
        <p style="font-size: 16px; margin-top: 20px;">Thank you,<br>SKIPPY</p>
      </td>
    </tr>
    <tr>
      <td style="background-color: #f5f5f5; padding: 15px; text-align: center; font-size: 12px; color: #888; border-radius: 0 0 10px 10px;">
        <p style="margin: 0;">&copy; ${new Date().getFullYear()} SKIPPY Team. All rights reserved.</p>
      </td>
    </tr>
    </table>
  </div>
